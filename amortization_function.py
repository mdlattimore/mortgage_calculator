def amortization(loan_amount, interest_rate, term):
    ...
    for index in range(1, term + 1):
        monthly_interest = round(((interest_rate / 100) * pb) / 12, 2)
        monthly_principal = round(pmt - monthly_interest, 2)
        ...
        if pmt > pb + monthly_interest:
            pmt = round(pb + monthly_interest, 2)
            monthly_interest = round(((interest_rate / 100) * pb) / 12, 2)
            monthly_principal = round(pmt - monthly_interest, 2)
            pb = round(pb - monthly_principal, 2)
